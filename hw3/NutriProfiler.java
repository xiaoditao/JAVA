...
public class NutriProfiler {
	...
	// Populates the recommendedNutrientsList using persons’ calculateEnergyRequirement() and 
	// calculateNutriRequirement() methods
	static void createNutriProfile(Person person) {
		person.recommendedNutrientsList.clear();
		int index = 0;
		float[] nutrientQuantity;
		if (person instanceof Female) {
			RecommendedNutrient energy = new RecommendedNutrient(ENERGY_NUTRIENT_CODE,((Female)person).calculateEnergyRequirement());
			nutrientQuantity = ((Female)person).calculateNutriRequirement();
			person.recommendedNutrientsList.add(energy);
		}else {
			RecommendedNutrient energy = new RecommendedNutrient(ENERGY_NUTRIENT_CODE,((Male)person).calculateEnergyRequirement());
			nutrientQuantity = ((Male)person).calculateNutriRequirement();
			person.recommendedNutrientsList.add(energy);
		}
		for (NutriEnum item : NutriEnum.values()) {
			RecommendedNutrient recommendedNutrient = new RecommendedNutrient(item.getNutrientCode(),nutrientQuantity[index]);
			index++;
